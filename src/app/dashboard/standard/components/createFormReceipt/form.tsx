...
function CreateFormReceipt() {
  ...
  const handleCreateInvoice = async (e:any) => {
    if(!customerName || !customerEmail || !depositTo || !products || !paymentMethod || !amount || !receiptDate) {
    ...
    await fetch("http://localhost:3001/standard/receipt",{
    method: "POST",
    mode: "cors", 
    cache: "no-cache", 
    credentials: "include", 
  ...
  return (
    ...
    <div className='w-fit h-fit flex-col justify-between'>
          <div className='flex relative'>
              <p className='ml-4 mt-4'>Receipt Date</p>
          </div>
        <input type='date' value={receiptDate} onChange={e=>setReceiptDate(e.target.value)} className="focus:shadow-[0_3px_10px_rgb(0,0,0,0.2)] hover:shadow-[0_3px_10px_rgb(0,0,0,0.2)] focus:outline-none bg-gray-50 border border-gray-300 text-gray-900 mb-6 text-sm rounded-lg  block w-[200px] p-2.5 ml-4 mt-4" />
    ...
    {/* message_on_invoice */}
      ...
      <div>
             <input onClick={e => handleCreateInvoice(e)} type="button" value="Create Receipt" placeholder='create invoice' className="hover:shadow-[0_3px_10px_rgb(0,0,0,0.2)] cursor-pointer bg-green-500 border border-gray-300 text-white mb-6 text-sm rounded-lg  block w-fit p-2.5 ml-4 mt-4"></input>
